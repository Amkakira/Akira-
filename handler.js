...
â•”â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•—
 â° âš ï¸ â± *USER BANNED!* â° âš ï¸ â±
â€”â—‰ *Notice ${messageNumber}/3 (Total: 3)*
â€”â—‰ Reason ${user.bannedReason ? `\n*:* ${user.bannedReason}` : '*Reason:* Unspecified'}
â€”â—‰ *Ø¥Ø°Ø§ ÙƒÙ†Øª ØªØ¹ØªÙ‚Ø¯ Ø£Ù† Ù‡Ø°Ø§ Ø®Ø·Ø£ ÙˆÙ„Ø¯ÙŠÙƒ Ø¯Ù„ÙŠÙ„ØŒ ÙÙŠÙ…ÙƒÙ†Ùƒ Ø§Ù„Ø§ØªØµØ§Ù„ Ø¨Ù…Ø§Ù„Ùƒ Ø§Ù„Ø±ÙˆØ¨ÙˆØª Ù„Ø§Ø³ØªØ¦Ù†Ø§Ù Ø§Ù„ØªØ¹Ù„ÙŠÙ‚.* â€”â—‰ *Ø§Ù„ØªÙˆØ§ØµÙ„ Ù…Ø¹ Ù…Ø§Ù„ÙƒÙ‡:* wa.me/22237979043
â•šâ•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
          ...
          if (botSpam.antispam && m.text && user && user.lastCommandTime && (Date.now() - user.lastCommandTime) < 5000 && !isROwner) {
            if (user.commandCount === 2) {
              ...
              if (remainingTime > 0) {
                const messageText = `*[ â„¹ï¸ ] Ø§Ù†ØªØ¸Ø± ${remainingTime} Ø«ÙˆØ§Ù†ÙŠ Ù‚Ø¨Ù„ Ø§Ø³ØªØ®Ø¯Ø§Ù… Ø£Ù…Ø± Ø¢Ø®Ø±*`;
                m.reply(messageText);
                return;
              } else {
                user.commandCount = 0;
        ...
        if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
          mconn.conn.reply(m.chat, `*[ â„¹ï¸ ] Ù„Ù‚Ø¯ Ù†ÙØ¯ Ø§Ù„Ù…Ø§Ø³ Ø§Ù„Ø®Ø§Øµ Ø¨ÙƒØŒ ÙŠÙ…ÙƒÙ†Ùƒ Ø´Ø±Ø§Ø¡ Ø§Ù„Ù…Ø²ÙŠØ¯ Ø¨Ø§Ø³ØªØ®Ø¯Ø§Ù… Ø§Ù„Ø£Ù…Ø±: ${usedPrefix}buyall*`, m);
          continue; 
        }
        if (plugin.level > _user.level) {
          mconn.conn.reply(m.chat, `*[ â„¹ï¸ ] It is required to have the level ${plugin.level}  to be able to use the command. Your current level is${_user.level},use the command  ${usedPrefix}lvl to raise your level with XP.*`, m);
          continue; 
	}
	      const extra = {
          match,
        ...
        try {
          ...
          if (m.limit) {
            m.reply('*[ ðŸ’Ž ] Will be used' + +m.limit + ' diamond(s) (limits).*');
          }
        }
        break;
      }
...
â”â”â”â”â”â”â”â”â”â”â¬£  ð˜¼ð™‰ð™ð™„ ð˜¿ð™€ð™‡ð™€ð™ð™€  â¬£â”â”â”â”â”â”â”â”â”
...
â”—â”â”â”â”â”â”â”â”â”â¬£  ð˜¼ð™‰ð™ð™„ ð˜¿ð™€ð™‡ð™€ð™ð™€  â¬£â”â”â”â”â”â”â”â”â”`.trim();
...
global.dfail = (type, m, conn) => {
  const msg = {
    rowner: '*[ â„¹ï¸ ] Ù„Ø§ ÙŠÙ…ÙƒÙ† Ø§Ø³ØªØ®Ø¯Ø§Ù… Ù‡Ø°Ø§ Ø§Ù„Ø£Ù…Ø± Ø¥Ù„Ø§ Ù…Ù† Ù‚Ø¨Ù„ Ù…Ø§Ù„Ùƒ Ø§Ù„Ø±ÙˆØ¨ÙˆØª.*',
    owner: '*[ â„¹ï¸ ] Ù„Ø§ ÙŠÙ…ÙƒÙ† Ø§Ø³ØªØ®Ø¯Ø§Ù… Ù‡Ø°Ø§ Ø§Ù„Ø£Ù…Ø± Ø¥Ù„Ø§ Ù…Ù† Ù‚Ø¨Ù„ Ù…Ø§Ù„Ùƒ Ø§Ù„Ø±ÙˆØ¨ÙˆØª.*',
    mods: '*[ â„¹ï¸ ] Ù„Ø§ ÙŠÙ…ÙƒÙ† Ø§Ø³ØªØ®Ø¯Ø§Ù… Ù‡Ø°Ø§ Ø§Ù„Ø£Ù…Ø± Ø¥Ù„Ø§ Ù…Ù† Ù‚Ø¨Ù„ Ø§Ù„Ù…Ø´Ø±ÙÙŠÙ† ÙˆÙ…Ø§Ù„Ùƒ Ø§Ù„Ø±ÙˆØ¨ÙˆØª.*',
    premium: '*[ â„¹ï¸ ] Ù„Ø§ ÙŠÙ…ÙƒÙ† Ø§Ø³ØªØ®Ø¯Ø§Ù… Ù‡Ø°Ø§ Ø§Ù„Ø£Ù…Ø± Ø¥Ù„Ø§ Ù…Ù† Ù‚Ø¨Ù„ Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù…ÙŠÙ† Ø§Ù„Ù…Ù…ÙŠØ²ÙŠÙ† ÙˆÙ…Ø§Ù„Ùƒ Ø§Ù„Ø±ÙˆØ¨ÙˆØª.*',
    group: '*[ â„¹ï¸ ] Ù„Ø§ ÙŠÙ…ÙƒÙ† Ø§Ø³ØªØ®Ø¯Ø§Ù… Ù‡Ø°Ø§ Ø§Ù„Ø£Ù…Ø± Ø¥Ù„Ø§ ÙÙŠ Ø§Ù„Ù…Ø¬Ù…ÙˆØ¹Ø§Øª.*',
    private: '*[ â„¹ï¸ ] Ù„Ø§ ÙŠÙ…ÙƒÙ† Ø§Ø³ØªØ®Ø¯Ø§Ù… Ù‡Ø°Ø§ Ø§Ù„Ø£Ù…Ø± Ø¥Ù„Ø§ ÙÙŠ Ø§Ù„Ø¯Ø±Ø¯Ø´Ø© Ø§Ù„Ø®Ø§ØµØ© Ù„Ù„Ø±ÙˆØ¨ÙˆØª.*',
    admin: '*[ â„¹ï¸ ] Ù„Ø§ ÙŠÙ…ÙƒÙ† Ø§Ø³ØªØ®Ø¯Ø§Ù… Ù‡Ø°Ø§ Ø§Ù„Ø£Ù…Ø± Ø¥Ù„Ø§ Ù…Ù† Ù‚Ø¨Ù„ Ù…Ø³Ø¤ÙˆÙ„ÙŠ Ø§Ù„Ù…Ø¬Ù…ÙˆØ¹Ø©.*',
    botAdmin: '*[ â„¹ï¸ ] Ù„Ø§Ø³ØªØ®Ø¯Ø§Ù… Ù‡Ø°Ø§ Ø§Ù„Ø£Ù…Ø±ØŒ Ù…Ù† Ø§Ù„Ø¶Ø±ÙˆØ±ÙŠ Ø£Ù† ÙŠÙƒÙˆÙ† Ø§Ù„Ø±ÙˆØ¨ÙˆØª Ù…Ø³Ø¤ÙˆÙ„Ø§ Ø¹Ù† Ø§Ù„Ù…Ø¬Ù…ÙˆØ¹Ø©.*',
    unreg: '*[ â„¹ï¸ ] Ù„Ø§Ø³ØªØ®Ø¯Ø§Ù… Ù‡Ø°Ø§ Ø§Ù„Ø£Ù…Ø±ØŒ ÙŠØ¬Ø¨ Ø£Ù† ØªÙƒÙˆÙ† Ù…Ø³Ø¬Ù„Ø§.*\n\n*[ ðŸ’¡ ] Use the command:* _#verify name.age_ *to register.*',
    restrict: '*[ â„¹ï¸ ] ØªÙ… ØªØ¹Ø·ÙŠÙ„ Ù‡Ø°Ø§ Ø§Ù„Ø£Ù…Ø± Ù…Ù† Ù‚Ø¨Ù„ Ù…Ø§Ù„Ùƒ Ø§Ù„Ø±ÙˆØ¨ÙˆØª.*',
  }[type];
  const aa = {quoted: m, userJid: conn.user.jid};
  const prep = generateWAMessageFromContent(m.chat, {extendedTextMessage: {text: msg, contextInfo: {externalAdReply: {title: '*[ âš  ] Warning*', body: 'JOHAN-BOT', thumbnail: imagen1, sourceUrl: 'https://chat.whatsapp.com/CjPwuDRKkUBQut8Pfkla'}}}}, aa);
