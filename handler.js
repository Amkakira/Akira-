...
if (botSpam.antispam && m.text && user && user.lastCommandTime && (Date.now() - user.lastCommandTime) < 5000 && !isROwner) {
  ...
  user.commandCount = 1
        ...
        if (!m.fromMem && m.text.match(/(اكيرا|بوته|بوت|تست|Akira | bot|zom100)/gi)) {
        let emot = pickRandom(["🎃", "❤", "💎", "⚡", "❓", "😎", "🙌", "⭐", "⁦👨🏻‍💻⁩", "🔥"])
...
export async function callUpdate(callUpdate) {
    ...
    //await this.sendContact(nk.from, data.map(([id, name]) => [id, name]), false, { quoted: callmsg })
     let vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;Yosef Ragab 👑;;;\nFN:Yosef Ragab 👑\nORG:Yosef Ragab 👑\nTITLE:\nitem1.TEL;waid=201020639057:+201020639057 \nitem1.X-ABLabel:Akira👑\nX-WA-BIZ-◤Nezuko - Bot ◢:[❗] كلم الرقم ده - المطور.\nX-WA-BIZ-NAME:Akira 👑\nEND:VCARD`
...
global.dfail = (type, m, conn) => {
    ...
    let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: '[ ⚠ ] تنبيه - إشعار', body: 'ᴛʜᴇ Nezuko - ʙᴏᴛ', thumbnail: imagen1, sourceUrl: 'https://github.com/BrunoSobrino/TheMystic-Bot-MD' }}}}, aa)
