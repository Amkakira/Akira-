let handler = async (m, { conn, text, command, usedPrefix }) => {
    ...
    let user = global.db.data.users[who] || {} // تعديل هنا
    let bot = global.db.data.settings[conn.user.jid] || {}
    let warntext = `*[❗] يرجي وضع منشن للشخص او الرد علي رساله له @منشن*\n\n*—◉ مثال:*\n*${usedPrefix + command} @${global.suittag}*`
    if (!who) return m.reply(warntext, m.chat, { mentions: conn.parseMention(warntext)}) // تعديل هنا
    user.warn = user.warn ? user.warn : 0 // تعديل هنا
    user.warn += 1
